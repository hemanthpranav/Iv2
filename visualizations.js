...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateAllCharts() {
    ...
    renderFuelChart(brushedPoints || currentFiltered);
    renderBar(brushedPoints || currentFiltered);
  }
  ...
  function renderScatter(data) {
    ...
    if (data.length === 0) {
      container.append("p").text("No data available").style("color", "red");
      return;
    }

    const width = 500, height = 350, margin = { top: 40, right: 30, bottom: 50, left: 60 };

    const x = d3.scaleLinear().domain(d3.extent(data, d => d.Horsepower)).nice().range([margin.left, width - margin.right]);
    const y = d3.scaleLinear().domain(d3.extent(data, d => d.MPG)).nice().range([height - margin.bottom, margin.top]);
    ...
    svg.append("text").attr("x", width / 2).attr("y", height - 5).attr("text-anchor", "middle").text("Horsepower");
    svg.append("text").attr("transform", "rotate(-90)").attr("x", -height / 2).attr("y", 15).attr("text-anchor", "middle").text("MPG");
  ...
  function renderFuelChart(data) {
    ...
    const x = d3.scaleBand().domain(barData.map(d => d.Manufacturer)).range([margin.left, width - margin.right]).padding(0.2);
    const y = d3.scaleLinear().domain([0, d3.max(barData, d => d.MPG)]).nice().range([height - margin.bottom, margin.top]);
    const svg = container.append("svg").attr("width", width).attr("height", height);
  ...
  function renderBar(data) {
    ...
    const y = d3.scaleLinear().domain([0, d3.max(barData, d => d.Weight)]).nice().range([height - margin.bottom, margin.top]);
    const svg = container.append("svg").attr("width", width).attr("height", height);
    svg.append("g").attr("transform", `translate(0,${height - margin.bottom})`).call(d3.axisBottom(x));
    svg.append("g").attr("transform", `translate(${margin.left},0)`).call(d3.axisLeft(y));
    svg.selectAll("rect").data(barData).enter().append("rect")
